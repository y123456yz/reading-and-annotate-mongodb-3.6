...
*/ 
//WiredTigerKVEngine._sizeStorer(针对表table:sizeStorer)   WiredTigerRecordStore._sizeStorer(每个集合有一个WiredTigerRecordStore类，_sizeStorer为该类的统计信息)
...
private:
    ...
    struct Entry { //下面的Map _entries;用到该结构
        ...
        WiredTigerRecordStore* rs;  // not owned  代表对应的集合WiredTigerRecordStore.uri
    };
    ...
    Map _entries; //WiredTigerSizeStorer._entries[].rs map表中记录所有的集合统计信息
