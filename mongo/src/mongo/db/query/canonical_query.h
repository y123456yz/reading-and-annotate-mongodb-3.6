...
*/
//Ö´ÐÐ¼Æ»® ÓÅ»¯Æ÷¿ÉÒÔ²Î¿¼https://yq.aliyun.com/articles/647563?spm=a2c4e.11155435.0.0.7cb74df3gUVck4
//¹æ·¶²éÑ¯£¬¼ûFindCmd::run  
...
private:
    ...
    std::unique_ptr<QueryRequest> _qr; //²éÑ¯ÖÐµÄÏà¹ØÏêÏ¸ÐÅÏ¢¼°²Ù×÷·ûµÈ¶¼¼ÇÂ¼ÔÚÕâÀïÃæ

    //CanonicalQuery._root  QuerySolutionNode.filter  Ò»¸öfilter¶ÔÓ¦Ò»¸öMatchExpression
    //ËùÓÐµÄQuerySolutionNode(´ú±íCanonicalQuery._rootÊ÷ÖÐµÄÒ»¸ö½Úµã£¬¶ÔÓ¦Ò»¸öMatchExpression)×é³ÉÒ»¿ÅÊ÷
    //²Î¿¼https://blog.csdn.net/baijiwei/article/details/78170387

    // _root points into _qr->getFilter()
    //²Î¿¼https://blog.csdn.net/baijiwei/article/details/78170387
    //MatchExpressionÊÇ½«filterËã×ÓÀïÃ¿¸öÂß¼­ÔËËã×ª»»³É¸÷¸öÀàÐÍµÄ±í´ïÊ½(GT,ET,LT,AND,OR...)£¬¹¹³ÉÒ»¸ö±í´ïÊ½tree½á¹¹£¬¶¥²ãrootÊÇÒ»¸öAndMatchExpression£¬Èç¹ûº¬ÓÐAND¡¢OR¡¢NOR£¬treeµÄÉî¶È¾Í+1. Õâ¸ö±í´ïÊ½tree»áÓÃ×öÒÔºó¹ýÂË¼ÇÂ¼¡£
    //¸³Öµ²Î¿¼CanonicalQuery::canonicalize->CanonicalQuery::init£¬¸Ãroot treeÍ¨¹ýMatchExpressionParser::parseÉú³É

    //QueryPlanner::plan->CanonicalQuery::toString()µ÷ÓÃ´òÓ¡ÇëÇóÓÅ»¯ºóµÄ²éÑ¯tree
    std::unique_ptr<MatchExpression> _root;   //CanonicalQuery._root 
    

    //projection : Ñ¡ÔñÊä³öÖ¸¶¨µÄfields£¬Àà±ÈSQLµÄselect  ÀýÈçdb.news.find( {}, { id: 1, title: 1 } )Ö»Êä³öidºÍtitle×Ö¶Î£¬µÚÒ»¸ö²ÎÊýÎª²éÑ¯Ìõ¼þ£¬¿Õ´ú±í²éÑ¯ËùÓÐ
    //projection¸³Öµ¼ûParsedProjection::make
    std::unique_ptr<ParsedProjection> _proj; 

    //collatorÊÇÓÃ»§¿ÉÒÔ×Ô¶¨ÒåµÄ³ýÁËByteComparator(Öð×Ö½Ú±È½ÏÅÅÐò)Ö®ÍâµÄ±È½Ï·½·¨£¬±ÈÈçÄÚÖÃµÄÖÐÎÄ±È½Ï¡£collatorÐèÒªºÍfilterÀïµÄÂß¼­±í´ïÊ½Ïà¹ØÁª(±ÈÈç$gt´óÓÚÔËËã)¡£
