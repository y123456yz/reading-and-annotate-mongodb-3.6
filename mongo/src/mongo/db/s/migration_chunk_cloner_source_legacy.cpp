...
Status MigrationChunkClonerSourceLegacy::_storeCurrentLocs(OperationContext* opCtx) {
    ...
    if (!idx) {
		//类似日志: W SHARDING [conn348058] Chunk move failed :: caused by :: IndexNotFound: can't find index with prefix { feature: "hashed" } in storeCurrentLocs for music.data_set
